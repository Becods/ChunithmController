...
class USB:
    ...
    def sendDecData(self, data, ui):    # 发送字符串
        if not device.is_plugged():
            ui.plainTextEdit_log.appendPlainText("USB设备已断开")
            return
...
class MainWindow(QMainWindow):
    ...
    def emptyCheck(self):
        ...
        if self.ui.lineEdit_threshold.text() == "":
            ...
            return 1
        if self.ui.lineEdit_infrared.text() == "":
            self.ui.plainTextEdit_log.appendPlainText("红外阈值不可为空!")
            return 1

    def on_read(self):  # 点击读取设备
        HID.sendDecData("GetKeys%", self.ui)
        ...
        while True:
            ...
            if current_time - start_time > timeout:
                self.ui.plainTextEdit_log.appendPlainText("数据接收超时, 请等待3秒后操作")
                break

    def on_readir(self):  # 点击读取红外
        HID.sendDecData("GetIr%", self.ui)
        start_time = time.time()
        while True:
            try:
                idata = HID.receiveDecData().split("/")
                if idata != 0:
                    irdata = list(idata)
                    irdata = ','.join(irdata)
                    self.ui.plainTextEdit_log.appendPlainText(irdata)
                    break
            except:
                pass
            current_time = time.time()
    ...
    def on_write(self): # 点击写入设备
        ...
        rdata.append(int(self.ui.lineEdit_infrared.text()))
        #print(rdata)
    ...
    def on_save(self):  # 点击保存配置文件
        ...
        writeConf.append(str(self.ui.lineEdit_infrared.text()))
    ...
    def on_gloves(self):
        HID.sendDecData("Gloves%", self.ui)
        self.ui.plainTextEdit_log.appendPlainText("切换至手套模式.")
    
    def on_hands(self):
        HID.sendDecData("Hands%", self.ui)
    ...
    def showConf(self, item):
        ...
        self.ui.lineEdit_infrared.setText(str(keyValue[41]))
    ...
    def showClientData(self, keyValue):
        ...
        self.ui.lineEdit_threshold.setText(keyValue[40][0])
        self.ui.lineEdit_infrared.setText(keyValue[40][1])
